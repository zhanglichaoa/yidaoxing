...
function x_name($name){
    return '*'.mb_substr($name, 1, mb_strlen($name, 'utf-8'), 'utf-8');
}
...
function getPrice(){
    // $resp=json_decode(file_get_contents('https://data.lianjinbox.com/realtime?id=10'),true);
    // $price=empty($resp['data']['Price'])?0:$resp['data']['Price'];
    // if(get_client_ip()=='127.0.0.1') $price=10;
    $price=10;
    return $price;
}
//字符串打散成数组
function strToArray($str,$arr=array()){
    if (strlen($str) == 1){
        return $arr;
    }
    $arr[] = substr($str,0,1);
    $str = substr($str,1);
    $arr = strToArray($str,$arr);
    return $arr;
}
//url生成二维码
function getQr($url){
...
function checkID($id){
    ...
    $arr = strToArray($id);
    $total = 0;
    $arr1 = array(7,9,10,5,8,4,2,1,6,3,7,9,10,5,8,4,2);
    foreach ($arr as $k=>$v){
        if ($k == 17){
            break;
        }
    ...
    $res = $total % 11;
    $ciphertext = array(1,0,'X',9,8,7,6,5,4,3,2);
    if ($ciphertext[$res] == substr($id,-1,1)){
        return true;
    }else{
        return false;
    }
...
function getIDInfo($id){
    $url = 'https://way.jd.com/yingyan/queryidcard?appkey=a9ae48ddba4816b7b0a40c6b05033605&id='.$id;
    $idInfo = Get($url);
    $info = json_decode($idInfo);
    if ($info->code == 10000){
        $res = $info->result;
        if ($res->resp->code == 0){
            $data = $res->data;
...
function api_post($api,$params,$api_url='app/',$debug=false,$json=true){
    $api_url=SERVER_URL.$api_url;
    $appserect='kqnCiAqhko7rqwufDB9FmG3R8KxJR2Ng';
    $params['appid']='app1000';
    $params['nonce_str']=rand(1000,9999);
    krsort($params);
    $strA='';
    ...
    $strB=substr(md5($strA),8,16);
    $strC=base64_urlSafeEncode($strB);
    $sign=hash_hmac('sha1',$strC,$appserect);
    $serverencodesign=base64_urlSafeEncode($sign);
    $params['encodesign']=$serverencodesign;
    $debug&&dump($params);
    $r=Post($params,$api_url.$api);
    $resp=$json?json_decode($r):$r;
    return $resp;
}
...
function send_sms($mobile,$code=null){
//    if(empty($code)){
        $code=rand(100000,999999);
//    }
    S($mobile.'sms_code',$code,3600);
    $num=S($mobile.'-'.date('Y-m-d'));
    $num=empty($num)?1:$num+1;
    $inum=S($mobile.'-s');
    if(!empty($inum)){
        $gets['SubmitResult']['code']=0;
        $gets['SubmitResult']['msg']='一分钟只能接收1条短信!';
        return $gets;
    }
    if($num>10){
        $gets['SubmitResult']['code']=0;
        $gets['SubmitResult']['msg']='今天已经达到接收短信上线,请明天再试!';
        return $gets;
    }
    S($mobile.'-'.date('Y-m-d'),$num,3600*24);
    S($mobile.'-s',1,60);
    $post_data = "account=C16091873&password=a7a4369b2474c2c22236c6de4ca5a50a&mobile=$mobile&content=".rawurlencode("短信动态码：$code 。您正在进行用户操作，请勿泄露短信动态码。");
    $gets =  xml_to_array(Post($post_data, "http://120.55.205.5/webservice/sms.php?method=Submit"));
    return $gets;
}
function cut_str($str,$len=50,$suffix="..."){
...
function isEmail($value){
    $match='/^([0-9A-Za-z\\-_\\.]+)@([0-9a-z]+\\.[a-z]{2,3}(\\.[a-z]{2})?)$/i';
    $v = trim($value);
    if(empty($v))
        return false;
...
function isMobileNum($value){
    ...
    if(empty($v))
        return false;
    $match='/^1[345789]{1}\d{9}$/';
    return preg_match($match,$v);
}
function isPWD($value,$minLen=8,$maxLen=16){
    $match='/^[0-9A-Za-z]{'.$minLen.','.$maxLen.'}$/'; 
    $v = trim($value);
    if(empty($v))
        return false;
    $r1=preg_match($match,$v);
    $match='/^[0-9]{0,}$/'; 
    $r2=!preg_match($match,$v);
    return $r1&&$r2;
}
function getPage($count,$limit=20){
    ...
    $Page->setConfig('prev','<<');
    $Page->setConfig('next','>>');
    $Page->setConfig('theme','<div class="pagination"><li class="paginate_button">%upPage%</li>%linkPage%<li class="paginate_button">%downPage%</li><li><a href="javascript:;">共 %totalRow% 条记录</a> </li></div>');
    return $Page->show();// 分页显示输出
}
function md516($p){
    return empty($p)?'':strtoupper(substr(md5($p."klc"),8,16));
}
